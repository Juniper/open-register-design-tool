...
// ------------------ ordt_rg_primary_base_regs_config_regs methods ------------------

ordt_rg_primary_base_regs_config_regs::ordt_rg_primary_base_regs_config_regs(uint64_t _m_startaddress, uint64_t _m_endaddress)
  : ordt_reg(_m_startaddress, _m_endaddress),
    lsb_field(0, 16, 0xffff, r_std, w_std),
    msb_field(16, 16, 0x0, r_std, w_std) {
}

int  ordt_rg_primary_base_regs_config_regs::write(const uint64_t &addr, const ordt_data &wdata) {
  #ifdef ORDT_PIO_VERBOSE
     std::cout << "--> write of reg ordt_rg_primary_base_regs_config_regs at addr="<< addr << ", data=" << wdata.to_string() << "\n";
  #endif
     if (this->hasStartAddress(addr)) {
        this->write(wdata);
        return 0;
  ...
  #ifdef ORDT_PIO_VERBOSE
     std::cout << "--> write to invalid address " << addr << " in reg ordt_rg_primary_base_regs_config_regs\n";
  #endif
     return 8;
}

void  ordt_rg_primary_base_regs_config_regs::write(const ordt_data &wdata) {
  std::lock_guard<std::mutex> m_guard(m_mutex);
  lsb_field.write(wdata);
  msb_field.write(wdata);
}

int  ordt_rg_primary_base_regs_config_regs::read(const uint64_t &addr, ordt_data &rdata) {
  #ifdef ORDT_PIO_VERBOSE
     std::cout << "--> read of reg ordt_rg_primary_base_regs_config_regs at addr="<< addr << "\n";
  #endif
     if (this->hasStartAddress(addr)) {
        this->read(rdata);
        return 0;
  ...
  #ifdef ORDT_PIO_VERBOSE
     std::cout << "--> read to invalid address " << addr << " in reg ordt_rg_primary_base_regs_config_regs\n";
  #endif
     rdata.clear();
     return 8;
}

void  ordt_rg_primary_base_regs_config_regs::read(ordt_data &rdata) {
  rdata.clear();
  for (uint64_t widx=0; widx<((m_endaddress - m_startaddress + 1)/4); widx++) rdata.push_back(0);
  lsb_field.read(rdata);
  msb_field.read(rdata);
...
// ------------------ ordt_rset_primary_base_regs methods ------------------

ordt_rset_primary_base_regs::ordt_rset_primary_base_regs(uint64_t _m_startaddress, uint64_t _m_endaddress)
  : ordt_regset(_m_startaddress, _m_endaddress),
    config_regs(_m_startaddress + 0x0, _m_startaddress + 0x3, 8, 0x4) {
  m_children.push_back(&config_regs);
}

void  ordt_rset_primary_base_regs::update_child_ptrs() {
  m_children.clear();
  m_children.push_back(&config_regs);
}

// ------------------ ordt_rset_primary methods ------------------

ordt_rset_primary::ordt_rset_primary(uint64_t _m_startaddress, uint64_t _m_endaddress)
  : ordt_regset(_m_startaddress, _m_endaddress),
    base_regs(_m_startaddress + 0x0, _m_startaddress + 0x1f) {
  m_children.push_back(&base_regs);
}

void  ordt_rset_primary::update_child_ptrs() {
  m_children.clear();
  m_children.push_back(&base_regs);
}
...
ordt_root::ordt_root(uint64_t _m_startaddress, uint64_t _m_endaddress)
  : ordt_regset(_m_startaddress, _m_endaddress),
    base_regs(_m_startaddress + 0x0, _m_startaddress + 0x1f),
    primary(_m_startaddress + 0x20, _m_startaddress + 0x3f) {
  m_children.push_back(&base_regs);
  m_children.push_back(&primary);
}
